 ...
 * To make use of this class, first set up how tokens should be
 * highlighted by invoking the getXXX and setXXX methods, then
 * pass in "chunks" of relevent input to be filtered.
 * Typically this will mean the section(s) of a form input that
 * has been tagged with [code][/code] tags.
 */
public class JavaCodeViewer {
    ...
    /**
     * Syntax highlights any java code in the input. In working with
     * this method it is helpful to know how lexers work in general.
     * <p>
     * The overall strategy is as follows:
     * The input is processed a character at a time, accompanied by a
     * state update.  When a valid Java token is detected, such as a
     * keyword, a string, a comment block, etc, said token gets "wrapped"
     * by htmlStart and htmlEnd tags.  Not everything is implemented
     * according to the Java Language Specifications.  For example, the
     * length of a valid number is left unchecked.
     * <p>
     * @param line string possibly containing Java code
    ...
    public final String javaCodeFilter(String line) {
        ...
        // for printing line numbers in right/left-justified positions
        // int numSpaces = String.valueOf(lineCount).length();
        // Odometer meter = new Odometer(numSpaces);
...
/*
        // first line
        meter.forwardFlip();
        int[] result = meter.getReading();
        buffer.append(intArraytoCharArray(result));
*/
        int i = 0;
        int head_idx = 0;
        int tail_idx = 0;
...
class Odometer {
    ...
    /**
     * @param size the number of digits the odometer has
     */
    public Odometer(int size) {
        try {
            if(size>1) {
                milege = new int[size];
                revolved = new boolean[size];
                for(int i=0; i<milege.length; i++) {
                    milege[i] = 0;
                    revolved[i] = false;
                }
            }
            else {
                throw new NumberFormatException("Odometer: number of digits must be larger than 0");
            }
        }
    ...
    public int[] forwardFlip() {
        milege[milege.length-1]++;
        if(milege[milege.length-1] == 10) {
            revolved[milege.length-1] = true;
            milege[milege.length-1] = 0;
        }
        for(int i=milege.length-1; i>=0; i--) {
            if(milege[i]==0 && i>0 && revolved[i]==true) {
                milege[i-1]++;
                revolved[i] = false;
                if(milege[i-1] == 10) {
                    revolved[i-1] = true;
