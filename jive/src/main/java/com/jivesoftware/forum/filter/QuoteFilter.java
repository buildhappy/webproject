...
/**
 * A ForumMessageFilter that highlights quoted messages
 * based upon the level of indentation - very similar to
 * how groups.google.com highlights quotes.
 *
 * @author Bruce Ritchie
 */
public class QuoteFilter extends ForumMessageFilter {
    ...
    public ForumMessageFilter clone(ForumMessage message){
        ...
        filter.quoteColors         = quoteColors;
        filter.reformatTextEnabled = reformatTextEnabled;
    ...
    /**
     * Returns the body of the message with quoted lines quoted appropriately.
     *
     * @return the filtered message body.
     */
    public String getBody() {
        return parseQuotes(message.getBody());
    }
    ...
    /**
     * This method takes a string which may contain quotes lines
     * and quotes the lines accordingly.  It does this
     * by adding the prefix line postfix according to the appropriate
     * quote level. An arbitrary level of quotes are available to be set, and
     * once those are used up it restarts from the beginning of the list
     *
    ...
    private String parseQuotes(String input) {
        ...
        StringBuffer sb = new StringBuffer(input.length() + 512);
        ...
        for (int c = 0; c < lines.size(); c++) {
            ...
            chars = StringUtils.replace(line, "&gt;", ">").toCharArray();
        ...
        for (int c = 0; c < lines.size(); c++) {
            ...
            for (int g = c+1; g < readAhead; g++) {
                ...
                if (isQuoted(chars) && countQuotes(chars) > count) {
                    // System.err.println("Too many quotes - " + new String(chars));
                    l.removeLast();
                    break;
                }
                // rule 2
                if (countWords(chars) > 2) {
                    // System.err.println("Too many words - " + new String(chars));
                    l.removeLast();
                    break;
                ...
                if (countQuotes(chars) == count) {
                    // System.err.println("same number of quotes - " + new String(chars));
                    l.removeLast();
                    break;
                }
                // rule 4
                if (!checkQuoteConsistency(l)) {
                    // System.err.println("Quote Inconsistent - " + new String(chars));
                    l.removeLast();
                    break;
                }
                // rule 5
            ...
            if (l.size() > 0) {
                ...
                StringBuffer sb = new StringBuffer(l.size()+1 * 80);
    ...
    private char[] getLineAsChar(ArrayList lines, int number) {
        ...
        return StringUtils.replace(line, "&gt;", ">").toCharArray();
    }
    ...
    private String stripQuotes(char[] line) {
        StringBuffer sb = new StringBuffer(new String(line));
        boolean start = false;
